...
/*
Plugin Name: Ads24 Lite - Wordpress Advertising Plugin
Plugin URI: https://wordpress.org/plugins/wp-ad-management/
Description: WP Ads Management Plugin With Awesome Features
...
// Require files
define( 'ADS24_LITE_ID', 'ADS24_LITE_plugin' );
require_once('lib/functions.php');
require_once('lib/ajax-functions.php');
require_once('lib/countries.php');
require_once('lib/ADS24_LITE_Model.php');
...
class Ads24Lite
{
	private $plugin_id = 'ADS24_LITE_plugin';
	private $plugin_version = '1.0';
	private $model;

	function __construct() {
		$this->model = new ADS24_LITE_Model();
		register_activation_hook(__FILE__, array($this, 'onActivate'));
		register_uninstall_hook(__FILE__, array('Ads24Lite', 'onUninstall'));
	}

	static function onUninstall()
	{
		$ver_opt = 'ADS24_LITE_plugin'.'_version';

		if ( get_option('ADS24_LITE_plugin_'.'installation') == 'yes' ) {
		...
		wp_clear_scheduled_hook( 'a24p_cron_job_views_stats' );

		wp_clear_scheduled_hook( 'a24p_cron_jobs' );
	...
	function onActivate()
	{
		...
		if ( (wp_next_scheduled( 'a24p_cron_job_views_stats' ) > time() + 24 * 60 * 60) === false ) {
		...
		a24pCreateCustomAdTemplates( true );
...
');
				...
				update_option($opt_trans.'_order_form', array(
					...
					'form_notice' 		=> 'All AdSlots are full. Contact us via the contact form.',
...
//add_action( 'send_headers', 'ADS24_LITE_add_cors_header' );
//function ADS24_LITE_add_cors_header()
//{
//	header('Access-Control-Allow-Origin: "*"');
//}

add_action('wp_enqueue_scripts', 'ADS24_LITE_add_stylesheet_and_script');
function ADS24_LITE_add_stylesheet_and_script()
{
	$rtl = (get_option('ADS24_LITE_plugin_rtl_support') == 'yes') ? 'rtl-' : null;
	wp_register_style('ads24_ads_pro_main_stylesheet', plugins_url('frontend/css/asset/'.$rtl.'style.css?v='.get_option('ADS24_LITE_plugin_version'), __FILE__));
	wp_enqueue_style('ads24_ads_pro_main_stylesheet');
	wp_register_style('ads24_ads_pro_user_panel', plugins_url('frontend/css/asset/'.$rtl.'user-panel.css', __FILE__));
	wp_enqueue_style('ads24_ads_pro_user_panel');
	if ( a24p_get_opt('other', 'optimization') == 'no' ) {
		...
		foreach ( $get_templates as $template ) {
			if ( strpos($template,'block-') !== false ) {
				wp_register_style('ads24_ads_pro_'.str_replace('.php','',$template).'_stylesheet', plugins_url('frontend/css/'.str_replace('.php','',$template).'.css', __FILE__));
				wp_enqueue_style('ads24_ads_pro_'.str_replace('.php','',$template).'_stylesheet');
			} else {
				wp_register_style('ads24_ads_pro_'.str_replace('.php','',$template).'_stylesheet', plugins_url('frontend/css/'.$rtl.str_replace('.php','',$template).'.css', __FILE__));
				wp_enqueue_style('ads24_ads_pro_'.str_replace('.php','',$template).'_stylesheet');
			}
		}
	} else {
		if ( file_exists('frontend/css/'.($rtl == 'rtl-' ? $rtl : '').'all.css') ) {
			wp_register_style('ads24_ads_pro_'.($rtl == 'rtl-' ? 'rtl_' : '').'template_stylesheet', plugins_url('frontend/css/'.($rtl == 'rtl-' ? $rtl : '').'all.css', __FILE__));
			wp_enqueue_style('ads24_ads_pro_'.($rtl == 'rtl-' ? 'rtl_' : '').'template_stylesheet');
		} else {
			// re-generate css
			ADS24_LITE_generate_css( ($rtl == 'rtl-' ? true : null) );
			wp_register_style('ads24_ads_pro_'.($rtl == 'rtl-' ? 'rtl_' : '').'template_stylesheet', plugins_url('frontend/css/'.($rtl == 'rtl-' ? $rtl : '').'all.css', __FILE__));
			wp_enqueue_style('ads24_ads_pro_'.($rtl == 'rtl-' ? 'rtl_' : '').'template_stylesheet');
		}
//		if ( $rtl == 'rtl-' ) {
//			wp_register_style('ads24_ads_pro_rtl_template_stylesheet', plugins_url('frontend/css/rtl-template.css.php', __FILE__));
//			wp_enqueue_style('ads24_ads_pro_rtl_template_stylesheet');
//		} else {
//			wp_register_style('ads24_ads_pro_template_stylesheet', plugins_url('frontend/css/template.css.php', __FILE__));
//			wp_enqueue_style('ads24_ads_pro_template_stylesheet');
//		}
	}
	wp_register_style('ads24_ads_pro_animate_stylesheet', plugins_url('frontend/css/asset/animate.css', __FILE__));
	wp_enqueue_style('ads24_ads_pro_animate_stylesheet');
	wp_register_style('ads24_ads_pro_chart_stylesheet', plugins_url('frontend/css/asset/chart.css', __FILE__));
	wp_enqueue_style('ads24_ads_pro_chart_stylesheet');
	if ( get_option('ADS24_LITE_plugin_'.'carousel_script') == 'bx' ) {
		wp_register_style('ads24_ads_pro_carousel_stylesheet', plugins_url('frontend/css/asset/jquery.bxslider.css', __FILE__));
		wp_enqueue_style('ads24_ads_pro_carousel_stylesheet');
	} else {
		wp_register_style('ads24_ads_pro_carousel_stylesheet', plugins_url('frontend/css/asset/a24p.carousel.css', __FILE__));
		wp_enqueue_style('ads24_ads_pro_carousel_stylesheet');
	}
	wp_register_style('ads24_ads_pro_materialize_stylesheet', plugins_url('frontend/css/asset/material-design.css', __FILE__));
	wp_enqueue_style('ads24_ads_pro_materialize_stylesheet');
	...
	wp_register_script('ads24_ads_pro_js_script', plugins_url('frontend/js/script.js', __FILE__), array('jquery','media-upload','thickbox'));
	wp_enqueue_script('ads24_ads_pro_js_script');
	wp_register_script('ads24_ads_pro_viewport_checker_js_script', plugins_url('frontend/js/jquery.viewportchecker.js', __FILE__), array('jquery','media-upload','thickbox'));
	wp_enqueue_script('ads24_ads_pro_viewport_checker_js_script');
	wp_register_script('ads24_ads_pro_chart_js_script', plugins_url('frontend/js/chart.js', __FILE__), array('jquery','media-upload','thickbox'));
	wp_enqueue_script('ads24_ads_pro_chart_js_script');
	if ( get_option('ADS24_LITE_plugin_'.'carousel_script') == 'bx' ) {
		wp_register_script('ads24_ads_pro_carousel_js_script', plugins_url('frontend/js/jquery.bxslider.js', __FILE__), array('jquery','media-upload','thickbox'));
		wp_enqueue_script('ads24_ads_pro_carousel_js_script');
	} else {
		wp_register_script('ads24_ads_pro_carousel_js_script', plugins_url('frontend/js/a24p.carousel.js', __FILE__), array('jquery','media-upload','thickbox'));
		wp_enqueue_script('ads24_ads_pro_carousel_js_script');
	}
	wp_register_script('ads24_ads_pro_simply_scroll_js_script', plugins_url('frontend/js/jquery.simplyscroll.js', __FILE__), array('jquery','media-upload','thickbox'));
	wp_enqueue_script('ads24_ads_pro_simply_scroll_js_script');
	if (strpos(get_page_link(), get_option('ADS24_LITE_plugin_ordering_form_url')) !== FALSE && get_option('ADS24_LITE_plugin_secret_key') !== '' && get_option('ADS24_LITE_plugin_publishable_key') !== '') {
		wp_register_script('ads24_ads_pro_stripe_js_script', 'https://js.stripe.com/v2/', array('jquery','media-upload','thickbox'));
		wp_enqueue_script('ads24_ads_pro_stripe_js_script');
	}
	if ( file_exists(plugin_dir_path( __FILE__ ).'frontend/js/custom.js') ) {
		wp_register_script('ads24_ads_pro_custom_js', plugins_url('frontend/js/custom.js', __FILE__), array('jquery','media-upload','thickbox'));
		wp_enqueue_script('ads24_ads_pro_custom_js');
	}
    
    
}

add_action( 'admin_enqueue_scripts', 'ADS24_LITE_add_admin_stylesheet_and_script' );
function ADS24_LITE_add_admin_stylesheet_and_script() {
	$rtl = (get_option('ADS24_LITE_plugin_rtl_support') == 'yes') ? 'rtl-' : null;
	if ( a24p_get_opt('other', 'optimization') == 'no' ) {
    ...
    wp_enqueue_style('ads24_ads_pro_fontawesome_cdn', 'https://stackpath.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css');
    
	wp_register_style('ads24_ads_pro_admin_materialize_stylesheet', plugins_url('frontend/css/asset/material-design.css', __FILE__));
	wp_enqueue_style('ads24_ads_pro_admin_materialize_stylesheet');
	wp_register_style('ads24_ads_pro_admin_stylesheet', plugins_url('frontend/css/asset/'.$rtl.'admin.css', __FILE__));
	wp_enqueue_style('ads24_ads_pro_admin_stylesheet');
	wp_register_style('ads24_ads_pro_admin_animate_stylesheet', plugins_url('frontend/css/asset/animate.css', __FILE__));
	wp_enqueue_style('ads24_ads_pro_admin_animate_stylesheet');
	wp_register_style('ads24_ads_pro_admin_switch_button_stylesheet', plugins_url('frontend/css/asset/jquery.switchButton.css', __FILE__));
	wp_enqueue_style('ads24_ads_pro_admin_switch_button_stylesheet');
	$getScripts = wp_scripts();
	if ( is_array($getScripts->in_footer) && array_search('jquery', $getScripts->in_footer) === false ) {
		wp_enqueue_script( 'jquery' );
	}
	...
	wp_register_script('ads24_ads_pro_admin_jquery_ui_js_script', plugins_url('frontend/js/jquery-ui.js', __FILE__));
	wp_enqueue_script('ads24_ads_pro_admin_jquery_ui_js_script');
	wp_register_script('ads24_ads_pro_admin_js_script', plugins_url('frontend/js/admin-script.js', __FILE__));
	wp_enqueue_script('ads24_ads_pro_admin_js_script');
	wp_register_script('ads24_ads_pro_admin_switch_button_js_script', plugins_url('frontend/js/jquery.switchButton.js', __FILE__));
	wp_enqueue_script('ads24_ads_pro_admin_switch_button_js_script');
	wp_register_script('ads24_ads_pro_tagsinput_js_script', plugins_url('frontend/js/jquery.tagsinput.min.js', __FILE__));
	wp_enqueue_script('ads24_ads_pro_tagsinput_js_script');
    
}

function a24pProGetOpt($name, $type) {
	$get_opt = 'ADS24_LITE_plugin_';
...
function ADS24_LITE_add_custom_stylesheet()
{
echo "<style>
	/* Custom ADS24_LITE Styles */
...
";
	...
	if ( $spaces ) {
		foreach ( $spaces as $space ) {
			if ( $space["font"] != '' && $space["font_url"] != '' ) {
				echo $space["font_url"];
				echo ".a24pProContainer.a24p-".$space["template"]. "{
				".$space["font"]."
			}";
			}
		}
	...
	echo "
	/* form */
	.a24pProOrderingForm { ".a24pProGetOpt('form_bg', 'bg')." ".a24pProGetOpt('form_c', 'c')." }
	.a24pProInput input,
	.a24pProInput input[type='file'],
	.a24pProSelectSpace select,
	.a24pProInputsRight .a24pInputInner,
	.a24pProInputsRight .a24pInputInner label { ".a24pProGetOpt('form_input_bg', 'bg')." ".a24pProGetOpt('form_input_c', 'c')." }
	.a24pProPrice  { ".a24pProGetOpt('form_price_c', 'c')." }
	.a24pProDiscount  { ".a24pProGetOpt('form_discount_bg', 'bg')." ".a24pProGetOpt('form_discount_c', 'c')." }
	.a24pProOrderingForm .a24pProSubmit,
	.a24pProOrderingForm .a24pProSubmit:hover,
	.a24pProOrderingForm .a24pProSubmit:active { ".a24pProGetOpt('form_button_bg', 'bg')." ".a24pProGetOpt('form_button_c', 'c')." }

	/* alerts */
	.a24pProAlert,
	.a24pProAlert > a,
	.a24pProAlert > a:hover,
	.a24pProAlert > a:focus { ".a24pProGetOpt('form_alert_c', 'c')." }
	.a24pProAlertSuccess { ".a24pProGetOpt('form_alert_success_bg', 'bg')." }
	.a24pProAlertFailed { ".a24pProGetOpt('form_alert_failed_bg', 'bg')." }

	/* stats */
	.a24pStatsWrapper .ct-chart .ct-series.ct-series-b .ct-bar,
	.a24pStatsWrapper .ct-chart .ct-series.ct-series-b .ct-line,
	.a24pStatsWrapper .ct-chart .ct-series.ct-series-b .ct-point,
	.a24pStatsWrapper .ct-chart .ct-series.ct-series-b .ct-slice.ct-donut { ".a24pProGetOpt('stats_views_line', 's')." }

	.a24pStatsWrapper  .ct-chart .ct-series.ct-series-a .ct-bar,
	.a24pStatsWrapper .ct-chart .ct-series.ct-series-a .ct-line,
	.a24pStatsWrapper .ct-chart .ct-series.ct-series-a .ct-point,
	.a24pStatsWrapper .ct-chart .ct-series.ct-series-a .ct-slice.ct-donut { ".a24pProGetOpt('stats_clicks_line', 's')." }

	/* Custom CSS */
	".get_option('ADS24_LITE_plugin_custom_css')."
</style>";
}
add_action('wp_enqueue_scripts', 'ADS24_LITE_add_custom_stylesheet');

function ADS24_LITE_load_color_picker() {
	wp_enqueue_style( 'wp-color-picker' );
	wp_enqueue_script( 'wp-color-picker' );
}
add_action( 'admin_enqueue_scripts', 'ADS24_LITE_load_color_picker' );
...
if ( version_compare( floatval( get_bloginfo( 'version' ) ), '4.7', '<' ) ) {
	...
	add_action('widgets_init',
			create_function('', 'return register_widget("ADS24_LITE_ShortCode_Init");')
	);

} else {

	// Add a filter to the wp 4.7 version attributes metabox
	function ADS24_LITE_register_widgets() {
		register_widget( 'ADS24_LITE_ShortCode_Init' );
	}

	add_action( 'widgets_init', 'ADS24_LITE_register_widgets' );

}

class ADS24_LITE_ShortCode_Init extends WP_Widget {
	public function __construct() {
		parent::__construct('a24p_shortcode_widget', 'ADS PRO - Shortcode', array(
...
//			echo ADS24_LITE_adslot($space_id, $max_width, $delay, $padding_top, $attachment, $crop, $if_empty);
		} else {
...
if ( function_exists('ADS24_LITE_sub_menu_agency') ) {
	class Ads24LiteAdsApiTemplate {
		...
		public static function get_instance() {
			...
			if( null == self::$instance ) {
				self::$instance = new Ads24LiteAdsApiTemplate();
			}
	...
	add_action( 'plugins_loaded', array( 'Ads24LiteAdsApiTemplate', 'get_instance' ) );
}

$Ads24LiteAdsPlugin = new Ads24Lite();
