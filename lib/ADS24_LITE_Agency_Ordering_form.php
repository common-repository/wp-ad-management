...
if ( isset($_POST) && isset($_POST['a24pProSubmit']) && isset($_GET['site_id']) || isset($_GET['oid']) && isset($_GET['cid']) ) {
...
<div class="a24pProOrderingForm">
	...
	<?php elseif ($getForm == "successAdded"): ?>
		...
		<div id="a24pSuccessProAgencyRedirect">
			...
			<form><input id="a24p_payment_agency_url" type="hidden" name="a24p_payment_url" value="<?php echo (isset($_SESSION['a24p_payment_url'])) ? $_SESSION['a24p_payment_url'] : '' ?>"></form>
		</div>
	<?php endif; ?>
		<div class="a24pProAlert a24pProAlertFailed" style="display: none">
			<strong><?php echo get_option($a24pTrans."alert_failed"); ?></strong>
			<p><?php echo get_option($a24pTrans."form_empty"); ?></p>
		</div>
	<?php
	...
	if ( isset($_GET['site_id']) && $_GET['site_id'] != '' && $model->countSpaces('agency_form', $_GET['site_id']) > 0 || $eid && $hasAccess ) {
		...
		if ( isset($_GET['oid']) && $_GET['oid'] != '' && a24p_ad($_GET['oid'], 'id') != null && a24p_space(a24p_ad($_GET['oid'], 'space_id'), 'status') == 'active' && !isset($_GET['cid']) ||
			...
			if ( a24p_ad($_GET['oid'], 'paid') == 1 || a24p_ad($_GET['oid'], 'paid') == 2 ) {
				...
				<small style="display: block;">
					<a href="<?php echo get_option('ADS24_LITE_plugin_agency_ordering_form_url') ?>" style="font-size:12px;font-weight: normal;">< <?php echo get_option($a24pTrans.'agency_back_button') ?></a>
				</small>
			<?php
			} else {
					...
					<div class="a24pProItems a24pGridGutter">
						...
						<div style="text-align:center;">
							...
							<small style="margin-top: -10px;display: block;text-align: center;">
								<a href="<?php echo get_option("ADS24_LITE_plugin_agency_ordering_form_url") ?>?site_id=<?php echo $_GET['site_id'] ?>" style="font-size:12px;font-weight: normal;text-decoration: none;">&lt; <?php echo get_option($a24pTrans."payment_return"); ?></a>
							</small>
						</div>
			...
			if ( !$eid || $eid && $hasAccess || $eid && a24p_role() == 'admin' ) {
				if ( $space_verify && $spaces || $eid && $hasAccess || $eid && a24p_role() == 'admin' ) {
					echo '
		<form action="" method="POST" enctype="multipart/form-data">
			<div class="a24pProInputs a24pProInputsLeft">
				...
				<h3>
					' . get_option($a24pTrans.($eid ? 'edit' : 'form')."_left_header") . ' <span class="a24pLoader a24pLoaderInputs" style="display:none;"></span><br>
					<small style="margin-top: -10px;display: block;">
						<a href="' . get_option('ADS24_LITE_plugin_agency_ordering_form_url') . '" style="font-size:12px;font-weight: normal;text-decoration: none;">< ' . get_option($a24pTrans . 'agency_back_button') . '</a>
					</small>
				</h3>
				<div class="a24pProInput'.($eid ? 'Hidden' : '').'" style="'.($eid ? 'display:none' : '').'">
					...
					<div class="a24pTemplatePreview a24pTemplatePreviewAgencyForm">
		...
		echo '
		<div class="a24pProContainerNew a24p-lite-col-3" style="display: block !important">
			<h3 style="font-size: 20px; text-align: center;">'.get_option($a24pTrans.'agency_title_form').'</h3>
			<div class="a24pProItems a24pGridGutter">
			'; ?>
...
<?php
		foreach ( $model->getSites('agency_form', 'active') as $site ) {
			if ( $model->countSpaces('agency_form', $site["id"]) > 0 ) {
				...
				echo '
					<div class="a24pProItem a24pSite a24pHidden animated fadeIn" data-animation="fadeIn" style="'.(( $site["thumb"] != '' ) ? 'background-image: url(&#39;'.a24p_upload_url(null, $site["thumb"]) : '' ).'&#39;);">
						<div class="a24pSiteInner">
							<div class="a24pSiteInnerBg"></div>
							<h3>
								<a href="'.$agency_form.((strpos($agency_form, '?')) ? '&' : '?').'site_id='.$site["id"].'">'.$site["title"].'</a>
								<small>'.$site["url"].'</small>
							</h3>
							<div class="a24pSiteActions">
								<div class="a24pSiteAction a24pSiteActionLeft"><a href="'.$site["url"].'" target="_blank">'.get_option($a24pTrans.'agency_visit_site').'</a></div>
								<div class="a24pSiteAction a24pSiteActionRight"><a href="'.$agency_form.((strpos($agency_form, '?')) ? '&' : '?').'site_id='.$site["id"].'">'.get_option($a24pTrans.'agency_buy_ad').'</a></div>
							</div>
						</div>
					</div>
				';
			...
			$.post("<?php echo admin_url("admin-ajax.php") ?>", {action:"a24p_required_inputs_callback",a24p_space_id:$("#ADS24_LITE_space_id").val(),a24p_get_required_inputs:1}, function(result) {
				...
				if ( result.indexOf("img") !== -1 ) { // show if img required
					...
					if ( a24pTemplatePreviewInner.attr('data-img') ) {
						$(".a24pTemplatePreviewAgencyForm .a24pProItemInner__img").css({"background-image" : "url(" + a24pTemplatePreviewInner.attr('data-img') + ")"});
					}
				} else {
		...
		(function($){
			if (input.files[0]) {
				...
				reader.onload = function (e) {
					$(".a24pTemplatePreviewAgencyForm .a24pProItemInner__img").css({"background-image" : "url(" + e.target.result + ")"});
				};
