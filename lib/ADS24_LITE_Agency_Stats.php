<?php
require_once dirname(__FILE__) . '/ADS24_LITE_Agency_Ordering_form.php'; // require ordering form if stats
$ad_id = $_GET['ADS24_LITE_id'];
$model = new ADS24_LITE_Model();
$statsFrom = (($model->a24pIntervalStats($ad_id, 'from') != null) ? $model->a24pIntervalStats($ad_id, 'from') : 0);
$countClicks = (($model->a24pIntervalStats($ad_id, 'from') != null) ? $model->a24pCountClicks($ad_id, $statsFrom[0]) : 0);
$countViews = (($model->a24pIntervalStats($ad_id, 'from') != null) ? $model->a24pCountViews($ad_id, $statsFrom[0]) : 0);
echo '
	...
	<div class="a24pStatsWrapperInner">
		...
		<div class="a24pStatsChart">
			<div class="a24pSumStats">
				'.get_option("ADS24_LITE_plugin_trans_stats_clicks").' <strong>'.$countClicks[0].'</strong>
			</div>
			<div class="a24pSumStats" style="margin: 0 5%">
				'.get_option("ADS24_LITE_plugin_trans_stats_views").' <strong>'.$countViews[0].'</strong>
			</div>
			<div class="a24pSumStats">
				'.get_option("ADS24_LITE_plugin_trans_stats_ctr").' <strong>'.(($countViews[0] > 1) ? number_format(($countClicks[0] / $countViews[0]) * 100, 2)." %" : " - " ).'</strong>
			</div>
		</div>';
...
echo '<h3 class="a24pHeaderClicks">'.get_option("ADS24_LITE_plugin_trans_stats_clicks").'</h3>';
