	...
	foreach ( $ads as $key => $ad ) {
		...
		if ( $ad['url'] != '' ) {
			...
			if ( isset($example) ) { // url to form if example in ad space
				echo '<a class="a24pProItem__url"'.(isset($rel) ? $rel : null).''.(isset($rel) ? $rel : null).' href="'.$form_url.'" target="_blank">';
			} else {
				if ( isset($type) && $type == 'agency' ) {
					echo '<a class="a24pProItem__url"'.(isset($rel) ? $rel : null).''.(isset($rel) ? $rel : null).' href="'.$agency_form.( (strpos($agency_form, '?')) ? '&' : '?' ).'ADS24_LITE_id='.$ad['id'].'&ADS24_LITE_url=1" target="_blank">';
				} else {
					echo '<a class="a24pProItem__url"'.(isset($rel) ? $rel : null).''.(isset($rel) ? $rel : null).' href="'.get_site_url().( (strpos(get_site_url(), '?')) ? '&' : '?' ).'ADS24_LITE_id='.$ad['id'].'&ADS24_LITE_url=1" target="_blank">';
				}
			}
		...
		echo '<div class="a24pProItemInner__img" style="background-image: url(&#39;'.a24p_crop_tool($crop, ((!isset($sid) && !isset($_POST['a24p_ad_id']) || isset($example)) ? $ad['img'] : a24p_upload_url(null, $ad['img'])), 125, 125).'&#39;)"></div>'; // -- ITEM -- IMG
