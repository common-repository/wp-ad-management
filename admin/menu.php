...
function ADS24_LITE_head_menu()
{
	?>
	<div class="wrap" style="display:none">
		<?php if ( get_option("ADS24_LITE_plugin_purchase_code") == '' || get_option("ADS24_LITE_plugin_purchase_code") == null ) {
		//	echo '
...
function ADS24_LITE_sub_menu_spaces()
{
	?>
	<div class="wrap" style="display:none">
		<?php if ( get_option("ADS24_LITE_plugin_purchase_code") == '' || get_option("ADS24_LITE_plugin_purchase_code") == null ) {
...
function ADS24_LITE_sub_menu_add_new_space()
{
	?>
	<div class="wrap" style="display:none">
		<?php a24pAddNewSpace(); ?>
...
function a24pAddNewSpace()
{

	if ( $_SERVER["REQUEST_METHOD"] == "POST" && $_POST["a24pProAction"] == 'updateSpace' ) {
		...
		if ( $_POST["name"] != '' ) {
			...
			if ( $cpc_status == 'isset' || $cpc_status == 'incorrect' || $cpm_status == 'isset' || $cpm_status == 'incorrect' || $cpd_status == 'isset' || $cpd_status == 'incorrect' ) {
				...
				if ( $_POST['max_items'] < $model->countAds($_GET['space_id']) || $cpc_status == 'incorrect' || $cpm_status == 'incorrect' || $cpd_status == 'incorrect' ) {
					...
					do_action( 'a24p-lite-updateSpace', $_POST, $_GET['space_id'] );

					if ( isset($_SESSION[a24p_get_opt('prefix').'a24p_space_'.$_GET['space_id']]) || isset($_SESSION[a24p_get_opt('prefix').'a24pProSpace'.$_GET['space_id']]) ) {
						unset($_SESSION[a24p_get_opt('prefix').'a24p_space_'.$_GET['space_id']]); // Reset cache
						unset($_SESSION[a24p_get_opt('prefix').'a24pProSpace'.$_GET['space_id']]); // Reset cache
					}

					echo '
					<div class="updated settings-error">
						<p><strong>AdSlot Saved.</strong></p>
					</div>';
				}
			} else {
	...
	} elseif ( $_SERVER["REQUEST_METHOD"] == "POST" && $_POST["a24pProAction"] == 'addNewSpace' ) {
		...
		if ( $_POST["name"] != '' ) {
			...
			if ( $cpc_status == 'isset' || $cpc_status == 'incorrect' || $cpm_status == 'isset' || $cpm_status == 'incorrect' || $cpd_status == 'isset' || $cpd_status == 'incorrect' ) {
				...
				if ( $cpc_status == 'incorrect' || $cpm_status == 'incorrect' || $cpd_status == 'incorrect' ) {
					...
					do_action( 'a24p-lite-addNewSpace', $_POST, $model->getTableName('spaces') );
					$_SESSION['a24p_space_status'] = 'space_added';

					echo '
					<div class="updated settings-error">
						<p><strong>Space saved.</strong> Click <a href="'.admin_url().'admin.php?page=a24p-lite-sub-menu-spaces">here</a> to show all spaces.</p>
					</div>';
				}
			} else {
...
function ADS24_LITE_sub_menu_add_new_ad()
{
	?>
	<div class="wrap" style="display:none">
		<?php if ( get_option("ADS24_LITE_plugin_purchase_code") == '' || get_option("ADS24_LITE_plugin_purchase_code") == null ) {
...
function a24pAddNewAd()
{
	$plugin_id = 'ADS24_LITE_plugin_';

	if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["a24pProAction"] == 'updateAd') {

		// validate form
		if ( isset($_GET['ad_id']) && a24p_space(a24p_ad($_GET['ad_id'], 'space_id'), 'template') != 'html' ) {
		...
		if ( $_POST["buyer_email"] != '' ) {
			...
			if ( isset($_POST["increase_limit"]) && $_POST["increase_limit"] != '' ) {
				if ( $_POST["increase_limit"] > 0 || $_POST["increase_limit"] < 0 ) { // increase / decrease limit
					if ( a24p_ad($_GET['ad_id'], 'ad_model') == 'cpd' ) {
						$time = time();
					...
					$increase_limit = a24p_ad($_GET['ad_id'], 'ad_limit');
				}
			} else {
				$increase_limit = null;
			}
...
//			var_dump(stripslashes( wp_filter_post_kses( addslashes( $_POST["html"] ) ) ));
			if ( (a24p_role() == 'user') ) {
				$status = ((get_option('ADS24_LITE_plugin_auto_accept') == 'no') ? 'pending' : null);
				$increase_limit = null;
			} else {
				$status = null;
			}
	...
	} elseif ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["a24pProAction"] == 'addNewAd') {

		// validate form
			if ( isset($_POST["space_id"]) && a24p_space($_POST["space_id"], 'template') != 'html' ) {
			foreach ( explode(',', str_replace('desc', 'description', $_POST['inputs_required'])) as $input ) {
				$error = FALSE;
				if ( $input == 'img' ) {
		...
		if ( isset($_POST["buyer_email"]) && $_POST["buyer_email"] != '' && isset($_POST["space_id"]) && $_POST["space_id"] != '' && isset($_POST["ad_model"]) && $_POST["ad_model"] != '' && isset($_POST["ad_limit_" . $_POST["ad_model"]]) && $_POST["ad_limit_" . $_POST["ad_model"]] != '' ) {
			...
			if ( (a24p_role() == 'user') ) {
				$status = ((get_option('ADS24_LITE_plugin_auto_accept') == 'no') ? 'pending' : 'active');
			} else {
				$status = 'active';
			}
			...
			$_SESSION['a24p_ad_status'] = 'ad_added';
...
function ADS24_LITE_sub_menu_creator()
{
	?>
	<div class="wrap" style="display:none">
		<?php if ( get_option("ADS24_LITE_plugin_purchase_code") == '' || get_option("ADS24_LITE_plugin_purchase_code") == null ) {
...
//			</div>';
		...
		<?php a24pCreateAdTemplate(); ?>
		<?php require_once 'creator.php'; ?>
	</div>
<?php
}

function a24pCreateAdTemplate($width = null, $height = null)
{
	if( $_SERVER["REQUEST_METHOD"] == "POST" && $_POST["a24pProAction"] == 'adCreator' ||
		$width != null && $height != null)
	{
			...
			$width != null && $height != null )
		{
			...
			$css_styles = '
/* -- START -- Reset */
#a24p-block-'.$ad_width.'--'.$ad_height.' h3,
#a24p-block-'.$ad_width.'--'.$ad_height.' a,
#a24p-block-'.$ad_width.'--'.$ad_height.' img,
#a24p-block-'.$ad_width.'--'.$ad_height.' span,
#a24p-block-'.$ad_width.'--'.$ad_height.' p {
	margin: 0;
	padding: 0;
	border: 0;
	border-radius: 0;
...
/* -- START -- TEMPLATE */
#a24p-block-'.$ad_width.'--'.$ad_height.'.a24pProContainerNew .a24pProItem,
#a24p-block-'.$ad_width.'--'.$ad_height.' .a24pProItemInner__thumb,
#a24p-block-'.$ad_width.'--'.$ad_height.' .a24pProAnimateThumb {
...
#a24p-block-'.$ad_width.'--'.$ad_height.' .a24pProAnimateThumb:before{
	content: "";
	display: block;
}

#a24p-block-'.$ad_width.'--'.$ad_height.' .a24pProItemInner__img {
	position:  absolute;
	width: 100%;
	max-height: '.$ad_height.'px;
	height: auto;
...
/* -- END -- TEMPLATE */';
			...
			if ($exists === false) {
				update_option('ADS24_LITE_plugin_custom_templates', $custom_templates.','.$ad_width.'--'.$ad_height);
			}
...
function ADS24_LITE_sub_menu_settings()
{
	?>
	<div class="wrap">
		<?php if ( get_option("ADS24_LITE_plugin_purchase_code") == '' || get_option("ADS24_LITE_plugin_purchase_code") == null ) {
...
//			</div>';
		...
		if ( is_multisite() && is_main_site() ) {
			if (get_site_option('ADS24_LITE_plugin_main_basedir') == null ||
				get_site_option('ADS24_LITE_plugin_main_baseurl') == null ||
				get_site_option('ADS24_LITE_plugin_order_form_url') == null ||
				get_site_option('ADS24_LITE_plugin_order_form_url') != get_option('ADS24_LITE_plugin_ordering_form_url') ||
				get_site_option('ADS24_LITE_plugin_agency_order_form_url') == null ||
				get_site_option('ADS24_LITE_plugin_agency_order_form_url') != get_option('ADS24_LITE_plugin_agency_ordering_form_url')) {
				$upload_dir = wp_upload_dir();
				update_site_option('ADS24_LITE_plugin_main_basedir', $upload_dir['basedir']);
				update_site_option('ADS24_LITE_plugin_main_baseurl', $upload_dir['baseurl']);
				update_site_option('ADS24_LITE_plugin_order_form_url', get_option('ADS24_LITE_plugin_ordering_form_url'));
				update_site_option('ADS24_LITE_plugin_agency_order_form_url', get_option('ADS24_LITE_plugin_agency_ordering_form_url'));
			}
		} ?>
		<?php a24pUpdateSettings(); ?>
...
function a24pUpdateSettings()
{
	$opt = 'ADS24_LITE_plugin_';

	if( $_SERVER["REQUEST_METHOD"] == "POST" &&
		$_POST["a24pProAction"] == 'updateSettings' &&
		$_POST["ordering_form_url"] != '' )
	{
		// Check Update
		update_option('ADS24_LITE_update_version', 100);
		// Counters
		if ( isset( $_POST['clicks_counter'] ) && $_POST['clicks_counter'] > 0 ) {
			update_option('ADS24_LITE_plugin_dashboard_clicks', $_POST['clicks_counter']);
		}
		if ( isset( $_POST['views_counter'] ) && $_POST['views_counter'] > 0 ) {
			update_option('ADS24_LITE_plugin_dashboard_views', $_POST['views_counter']);
		}
		// Settings
		...
		if( $_SERVER["REQUEST_METHOD"] == "POST" && $_POST["a24pProAction"] == 'updateSettings' && $_POST["ordering_form_url"] == '' ||
			$_SERVER["REQUEST_METHOD"] == "POST" && $_POST["a24pProAction"] == 'updateSettings' && $_POST["ordering_form_url"] == '#' ) {
			echo '
			<div class="updated settings-error">
...
function ADS24_LITE_sub_menu_translations()
{
	?>
	<div class="wrap" style="display:none">
		<?php a24pUpdateTranslations(); ?>
		<?php require_once 'translations.php'; ?>
	</div>
<?php
}

function a24pUpdateTranslations()
{
	$opt = 'ADS24_LITE_plugin_';
	$opt_trans = 'ADS24_LITE_plugin_trans_';

	if( $_SERVER["REQUEST_METHOD"] == "POST" && $_POST["a24pProAction"] == 'updateTranslations')
	{
		// Translations
		...
		// additional actions
		do_action( 'a24p-lite-update-translations', $_POST, $opt_trans);
		echo '
		<div class="updated settings-error">
...
function ADS24_LITE_sub_menu_users()
{
	?>
	<div class="wrap" style="display:none">
		<?php $model = new ADS24_LITE_Model(); $model->getAdminAction() ?>
		<?php require_once 'users-manager.php'; ?>
	</div>
<?php
}

function ADS24_LITE_sub_menu_cron()
{
	?>
	<div class="wrap" style="display:none">
	<?php	$model = new ADS24_LITE_Model(); $model->getAdminAction() ?>
...
function ADS24_LITE_sub_menu_ab_tests()
{
	?>
	<div class="wrap" style="display:none">
		<?php $model = new ADS24_LITE_Model(); $model->getAdminAction() ?>
...
function ADS24_LITE_sub_menu_affiliate()
{
	?>
	<div class="wrap" style="display:none">
		<?php $model = new ADS24_LITE_Model(); $model->getAdminAction() ?>
...
function ADS24_LITE_sub_menu_updates()
{
	?>
	<div class="wrap" style="display:none">
		<?php $model = new ADS24_LITE_Model(); $model->getAdminAction() ?>
...
function ADS24_LITE_create_menu()
{
	if ( is_multisite() && is_main_site() || !is_multisite() ) {
		$count = strlen(get_option('ADS24_LITE_plugin_purchase_code'));
		$icon_url = plugins_url('../frontend/img/ads24-icon.png', __FILE__);
		$role = ((a24p_role() == 'admin') ? 'a' : 'u');
		$affiliate_name = a24p_get_trans('affiliate_program', 'affiliate');
        
   
	 	add_menu_page('Ads24 Lite - Admin Panel', 'Ads24 Lite', (($role == 'a') ? 'read' : 'manage_options'), 'a24p-lite-sub-menu', 'ADS24_LITE_head_menu', 'dashicons-megaphone');
		add_submenu_page('a24p-lite-sub-menu', 'Add new Ad', 'Add new Ad', 'read', 'a24p-lite-sub-menu-add-new-ad', 'ADS24_LITE_sub_menu_add_new_ad');
        add_submenu_page('a24p-lite-sub-menu', 'Add new AdSlot', 'Add new AdSlot', (($role == 'a') ? 'read' : 'manage_options'), 'a24p-lite-sub-menu-add-new-space', 'ADS24_LITE_sub_menu_add_new_space');
        add_submenu_page('a24p-lite-sub-menu', 'Control Panel', 'Control Panel', (($role == 'a') ? 'read' : 'manage_options'), 'a24p-lite-sub-menu-spaces', 'ADS24_LITE_sub_menu_spaces'); 
		add_submenu_page('a24p-lite-sub-menu', 'Custom Ad Size Tool', 'Custom Ad Size Tool', (($role == 'a') ? 'read' : 'manage_options'), 'a24p-lite-sub-menu-creator', 'ADS24_LITE_sub_menu_creator');
		add_submenu_page('a24p-lite-sub-menu', 'Ads Scheduling', 'Ads Scheduling', (($role == 'a') ? 'read' : 'manage_options'), 'a24p-lite-sub-menu-cron', 'ADS24_LITE_sub_menu_cron');
		add_submenu_page('a24p-lite-sub-menu', 'A/B Tests', 'A/B Tests', (($role == 'a') ? 'read' : 'manage_options'), 'a24p-lite-sub-menu-ab-tests', 'ADS24_LITE_sub_menu_ab_tests');
		add_submenu_page('a24p-lite-sub-menu', (($role == 'a') ? 'Users Manager' : 'Your Ads'), (($role == 'a') ? 'Users Manager' : 'Your Ads'), 'read', 'a24p-lite-sub-menu-users', 'ADS24_LITE_sub_menu_users');
		add_submenu_page('a24p-lite-sub-menu', 'Settings', 'Settings', (($role == 'a') ? 'read' : 'manage_options'), 'a24p-lite-sub-menu-opts', 'ADS24_LITE_sub_menu_settings');
		add_submenu_page('a24p-lite-sub-menu', 'Translations', 'Translations', (($role == 'a') ? 'read' : 'manage_options'), 'a24p-lite-sub-menu-trans', 'ADS24_LITE_sub_menu_translations');
	}
}
add_action('admin_menu', 'ADS24_LITE_create_menu');

function ADS24_LITE_pages_body_class( $classes ) {
	$classes[] = 'ADS24_LITE_pages_body_class';
	return $classes;
}
add_filter( 'admin_body_class ', 'ADS24_LITE_pages_body_class' );
